  ...
  <div class="l-entry__inner l-inner">
    <article class="p-entry">
      <div class="p-entry__header">
        <div class="p-entry__meta">
          <time class="p-entry__published" datetime="<?php the_time('c'); ?>"><span>公開日</span>
             ...
             if($this_category) {
              $this_category_color = get_field( 'color', 'category_' . $this_category[0]->term_id); 
               $this_category_name  = $this_category[0]->name;
                echo '<p class="p-fv-card__category" style= "' . esc_attr(
    ...
    if ($post_tags) {
        ...
        foreach($post_tags as $tag) {
            echo '<a href="' . get_tag_link($tag->term_id) . '" class="c-tag">' . $tag->name . '</a> ';
        }
        echo '</div>';
    }
    ?>
